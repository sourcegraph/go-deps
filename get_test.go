...
func TestRead(t *testing.T) {
	...
	tests := []struct {
		...
		{"imports_github", Package{
			...
			Name:       "imports_github",
			Goroot:     false, Standard: false,
			Imports:      []string{"github.com/sourcegraph/go-deps"},
			Deps:         []string{"github.com/sourcegraph/go-deps", "runtime", "unsafe"},
			DepsNotFound: []string{"github.com/sourcegraph/go-deps"},
			Incomplete:   true,
			Error:        nil,
			DepsErrors: []*PackageError{
				{
					ImportStack: []string{"imports_github", "github.com/sourcegraph/go-deps"},
					Pos:         "testdata/src/imports_github/imports_github.go:3:8",
					Err:         "cannot find package \"github.com/sourcegraph/go-deps\"",
				},
			},
		}},
